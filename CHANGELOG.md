...
**Добавлено**:
- Сортировка таблицы по клику на заголовок любой колонки (повторный клик меняет направление).
- Сортировка по умолчанию — по дате отсечки дивиденда (ближайшие сверху).
- Возможность перемещать колонки перетаскиванием заголовка.
- Сохранение порядка колонок в `localStorage` браузера (восстанавливается при перезагрузке).

**Технические детали**:
- Таблица строится динамически из массива описаний колонок (`COLUMNS`) в JS.
- Сортировка по дате использует ISO-формат `DividendDateSort` (добавлен в данные WebSocket).
- Перетаскивание реализовано через нативный HTML5 drag-and-drop API.
- Подсказка про сортировку и перемещение добавлена в легенду под таблицей.

**Известные ограничения**:
- Сортировка сбрасывается на дефолтную (по дате) при перезагрузке страницы — сохраняется только порядок колонок.

## [0.4.5] - 2026-09-25

**Добавлено**:
- Колонка «Дата отсечки» в таблице — показывает последний день покупки акции с дивидендом (в формате «8 окт»).
- Колонка «Див.дох.%» в таблице — дивидендная доходность (`dividendNet / priceStock`).
- Передача `DividendDate` и `DividendYield` в WebSocket-сообщениях.

**Технические детали**:
- Поле `DividendYield` добавлено в `Result` калькулятора.
- В `updater.go` добавлена функция `formatDatePtr` для форматирования даты отсечки в короткий формат с русскими месяцами.
- HTML-таблица расширена до 12 колонок.
- В колонке «Див.чист» добавлен символ ₽ (косметическое изменение).

**Известные ограничения**:
- Год в дате отсечки не выводится — предполагается, что дата всегда в обозримом будущем.

## [0.4.4] - 2026-09-25

**Добавлено**:
- Параметр `margin_multiplier` в `config.yaml` — множитель ГО для корректного учёта уровня риска клиента (КПУР: 0.5, КСУР: 1.0).
- Бейдж уровня риска (КПУР/КСУР) рядом с заголовком на сайте с всплывающей подсказкой.
- HTTP-эндпоинт `/api/config`, возвращающий JSON с настройками клиента.

**Исправлено**:
- ГО фьючерсов теперь соответствует фактическому уровню риска клиента. Ранее использовалось значение API по стандартному уровню (КСУР), из-за чего для КПУР-клиентов `ГО/акц` был завышен в 2 раза, а доходность — занижена.

**Технические детали**:
- Множитель применяется в `internal/api/service.go` в методе `GetFutureGO` — единообразно для сохранения в БД и для расчётов.
- Значение по умолчанию — 1.0 (КСУР), если параметр отсутствует в `config.yaml`.
- Для КПУР указывайте `margin_multiplier: 0.5`.

**Известные ограничения**:
- Уровень риска задаётся вручную. Автоопределение через API — в планах (см. `TODO.md`).
